...
public class TodoController {
    ...
    public Mono<String> addPatch(ServerHttpRequest request,
        ...
        return Mono.just(entity).publishOn(dbThreadPool.daoInstance()).map(v -> {
    ...
    public Mono<String> search(ServerHttpRequest request,
        ...
        return Mono.just(session.getUser(request)).publishOn(dbThreadPool.daoInstance()).map(v -> {
            ...
            return todoService.searchTodoGroupList(v.getId(), offset, count, statusVal, startTime, endTimeVal, sort)
                .toString();
        });
    }
    ...
    public Mono<String> todoList(ServerHttpRequest request, @RequestParam(value = "groupId") Long groupId) {
    ...
    public Mono<String> update(ServerHttpRequest request, @Valid @RequestBody Todo todo) {
        return Mono.just(todo).publishOn(dbThreadPool.daoInstance()).map(v -> {
    ...
    public Mono<String> updateTodoGroup(@Valid @RequestBody TodoGroupRequest todoGroupRequest) {
        return Mono.just(todoGroupRequest).publishOn(dbThreadPool.daoInstance()).map(v -> {
            TodoGroup todoGroup = new TodoGroup();
    ...
    public Mono<String> countTodoGroup(ServerHttpRequest request,
        ...
        return Mono.just(session.getUser(request)).publishOn(dbThreadPool.daoInstance()).map(
            v -> Result.success(todoGroupMapper.selectCountByUid(v.getId(), status, startTime, endTime)).toString());
    }
}
