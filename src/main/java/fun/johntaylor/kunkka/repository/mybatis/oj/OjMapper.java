...
public interface OjMapper {
    String COLUMNS =
        "id, pid, uid, name, difficulty, oj_type as ojType, type, pre_time as preTime, use_time as useTime, standalone, study, link, ans_link as ansLink, importance, create_time as createTime, update_time as updateTime, status";

    @Select("select " + COLUMNS + " from t_oj where id=#{id}")
    Oj select(Long id);

    @Insert("insert into t_oj(id, pid, uid, name, difficulty, oj_type, type, pre_time, use_time, standalone, study, link, ans_link, importance, create_time, update_time, status) values(#{id}, #{pid}, #{uid}, #{name}, #{difficulty}, #{ojType}, #{type}, #{preTime}, #{useTime}, #{standalone}, #{study}, #{link}, #{ansLink}, #{importance}, #{createTime}, #{updateTime}, #{status})")
